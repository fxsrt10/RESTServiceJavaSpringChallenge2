...
//------------------------------------------------------------------------
	public Double getMortgageAmountRef(String id) {
		Double mortgageAmount = null;
		try {

			System.out.println("test");
			ResultSet rs = stmt.executeQuery("SELECT Original_Mortgage_Amount FROM refinance WHERE ID =" + id);
			while (rs.next())
				mortgageAmount = Double.valueOf(rs.getString(1).replace(",", ""));
			System.out.println(mortgageAmount);
	...
	public Double getAverageMortgageAmountRef() {
		Double average = null;
		try {
	...
	public List<Purchase> getAverageMortgageAmountByCityRef() {
		List<Purchase> purchases = new ArrayList<>();
		try {

			System.out.println("test");
			ResultSet rs = stmt.executeQuery("SELECT Property_City, AVG(Original_Mortgage_Amount) FROM refinance GROUP BY Property_City");
			while (rs.next()){
				Purchase p = new Purchase();
				p.setOrigMortgageAmount(Double.valueOf(rs.getDouble(2)));
				p.setPropertyCity(rs.getString(1));
	...
	public List<Purchase> getPurchasesByCityRef(String city) {
		List<Purchase> purchases = new ArrayList<>();

		try {

			System.out.println("test");
			ResultSet rs = stmt.executeQuery("SELECT * FROM refinance WHERE Property_City = " +"'"+ city +"'");
			while (rs.next()){
				System.out.println("---------");
				System.out.println(rs.getInt(1));
				Purchase p = new Purchase();
	...
	public List<Purchase> getPurchasesRef() {
		List<Purchase> purchases = new ArrayList<>();
	...
	public List<Purchase> getPurchasesByZipRef(String zip) {
		List<Purchase> purchases = new ArrayList<>();

		try {

			System.out.println("test");
			ResultSet rs = stmt.executeQuery("SELECT * FROM refinance WHERE Property_Zip = " +"'"+ zip +"'");
			while (rs.next()){
				System.out.println("---------");
				System.out.println(rs.getInt(1));
				Purchase p = new Purchase();
